...
//*******************STEP:3 ESTABLISH CONNECTION TO THE DRIVER*******************************//
			...
			////////////////////////////////////////////////////
			prest = con.prepareStatement("SELECT BCC,BCC_BM_AVERAGE,BCC_CM_AVERAGE,BCC_TOTAL_AVERAGE,BCC_ENTRYMONTH,BCC_ENTRYDATE FROM bcc_procuriment WHERE BCC_ENTRYDATE= ? ORDER BY BCC");
...
<br/>	
<hr>	
<div align="center" style="text-decoration:underline;font-size:12pt;font-family:calibri;text-decoration:underline;font-size:12pt;color:black;font-weight:bold;">AGGREGATE TOTALS OF BCC PROCUREMENT DETAILS ON THE PARTICULAR DAY</div><br/>	
<table border="1" bordercolor="black" align="center">
<tr bgcolor="yellow">
...
try{
		    ...
		    prest = con.prepareStatement("SELECT SUM(BCC_BM_AVERAGE)TOTAL_BM_AVERAGE FROM bcc_procuriment   WHERE BCC_ENTRYDATE= ? ORDER BY BCC");
			prest.setString(1, Y1);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
		//********************************************************************************//
...
<%		 
		    prest = con.prepareStatement("SELECT SUM(BCC_CM_AVERAGE)TOTAL_CM_AVERAGE FROM bcc_procuriment WHERE BCC_ENTRYDATE= ? ORDER BY BCC");
			prest.setString(1, Y1);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
		//********************************************************************************//
...
<%		 
		    prest = con.prepareStatement("SELECT SUM(BCC_TOTAL_AVERAGE)TOTAL_AVERAGE FROM bcc_procuriment WHERE BCC_ENTRYDATE= ? ORDER BY BCC");
			prest.setString(1, Y1);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
		//********************************************************************************//
