...
<p style="color:black;font-family:calibri;font-weight:bold;">MONTH : <%=Y2 %></p>
...
//*******************STEP:3 ESTABLISH CONNECTION TO THE DRIVER*******************************//
			...
			////////////////////////////////////////////////////
			prest = con.prepareStatement("SELECT * FROM center_profitdetails WHERE BCC= ? AND MONTHPRO = ? ORDER BY CENTERNAME");
...
try{
		    ...
		    prest = con.prepareStatement("SELECT SUM(JOINTACCOUNTBALANCE)TOTALJOINTACCOUNTBALANCE FROM center_profitdetails WHERE BCC= ? AND MONTHPRO = ? ORDER BY CENTERNAME");
			prest.setString(1, Y1);	
			prest.setString(2, Y2);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
...
<%		 
		    prest = con.prepareStatement("SELECT SUM(FDAMOUNT)TOTALFDAMOUNT FROM center_profitdetails WHERE BCC= ? AND MONTHPRO = ? ORDER BY CENTERNAME");
			prest.setString(1, Y1);	
			prest.setString(2, Y2);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
...
<%		 
		    prest = con.prepareStatement("SELECT SUM(TOTAL)TOTALAMOUNT FROM center_profitdetails WHERE BCC= ? AND MONTHPRO = ? ORDER BY CENTERNAME");
			prest.setString(1, Y1);	
			prest.setString(2, Y2);	
			result = prest.executeQuery();
				System.out.println("Querie Successfully Executed");
