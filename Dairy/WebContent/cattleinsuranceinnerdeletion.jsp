...
<title>CATTLE INSURANCE INNER DELETION</title>
...
//*******************STEP:3 ESTABLISH CONNECTION TO THE DRIVER*******************************//
		...
		System.out.println("CONNECTION FROM THE DATA BASE IS ESTABLISHED");
            ...
            prest = con.prepareStatement("select CI_DATE_OF_ENTRY FROM cattle_i");
	...
	while(result.next()) {
	 ...
	 String s1 = result.getString("CI_DATE_OF_ENTRY");
	 a2.add(s1);
	}
	...
	//***********************************************************//
	 ...
	 prest = con.prepareStatement("select CENTERNAME FROM cattle_i ");
		...
		System.out.println(" ANNUM Querie Successfully Executed");
			...
			if(a4.size()!=0){
				...
				prest = con.prepareStatement("DELETE  FROM cattle_i WHERE CENTERNAME=? AND CI_DATE_OF_ENTRY = ? AND CI_DDNUMBER=?");
       ...
       System.out.println("data deleted successfully");				   			
			...
			s10.setAttribute("cname5",p);
			 ...
			 RequestDispatcher rd2 = request.getRequestDispatcher("Successfuldelete2.jsp");
